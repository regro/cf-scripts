...
yaml_rebuild = MigrationYaml(yaml_contents={"foo": "hello world"}, name="hi")
yaml_rebuild.cycles = []
yaml_rebuild.filter = lambda x: False

yaml_rebuild_nb = MigrationYaml(yaml_contents={"foo": "hello world", 'build_number': 0}, name="hi")
yaml_rebuild_nb.cycles = []
yaml_rebuild_nb.filter = lambda x: False
...
def test_yaml_migration(m, inp, output, kwargs, prb, mr_out, should_filter, tmpdir):
    ...
    with open(os.path.join(tmpdir, ".ci_support/migrations/hi.yaml")) as f:
        assert f.read() == safe_dump(m.yaml_contents)
