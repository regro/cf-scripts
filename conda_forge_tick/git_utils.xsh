...
def get_repo(attrs, branch, feedstock=None, protocol='ssh',
    ...
    with indir(feedstock_dir):
        ...
        # git pull @(upstream) master -s recursive -X theirs --no-edit -q
        # always run upstream master
        git remote add upstream @(upstream) -q 
        git fetch upstream master -q 
        git reset --hard upstream/master -q 
...
def refresh_pr(pr_json: LazyJson, gh=None, dry_run=False):
    if gh is None:
        gh = github3.login($USERNAME, $PASSWORD)
...
@backoff.on_exception(backoff.expo,
  (RequestException, Timeout),
  max_time=MAX_GITHUB_TIMEOUT)
def close_out_labels(pr_json: LazyJson, gh=None, dry_run=False):
    if gh is None:
        gh = github3.login($USERNAME, $PASSWORD)
