...
def ensure_gh(
    ctx: GithubContext, gh: Optional[github3.GitHub]
) -> github3.GitHub:
    if gh is None:
...
def feedstock_url(fctx: FeedstockContext, protocol: str = "ssh",) -> str:
    ...
    if protocol == "http":
        ...
        msg = (
            "Unrecognized github protocol {0!r}, must be ssh, http, or https."
        )
...
def get_repo(
    ...
    if pull_request or fork:
        ...
        if repo is None:
            fctx.attrs[
                "bad"
            ] = f"{fctx.package_name}: does not match feedstock name\n"
            return False
    ...
    if fork:
        try:
            ...
            fork_repo = None
        if fork_repo is None or (
            hasattr(fork_repo, "is_null") and fork_repo.is_null()
        ):
    ...
    feedstock_dir = os.path.join(
        ctx.rever_dir, fctx.package_name + "-feedstock"
    )
...
def delete_branch(
    ctx: GithubContext, pr_json: LazyJson, dry_run: bool = False
) -> None:
...
def refresh_pr(
    ...
    if not pr_json["state"] == "closed":
        if dry_run:
            ...
            if pr_obj_d["state"] == "closed" and pr_obj_d.get(
                "merged_at", False
            ):
...
def is_github_api_limit_reached(
    e: github3.GitHubError, gh: github3.GitHub
) -> bool:
