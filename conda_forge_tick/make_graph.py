...
def update_graph_pr_status(dry_run, gx: nx.DiGraph) -> nx.DiGraph:
    failed_refresh = 0
    succeeded_refresh = 0
...
def close_labels(dry_run, gx: nx.DiGraph) -> nx.DiGraph:
    failed_refresh = 0
    succeeded_refresh = 0
    gh = "" if dry_run else github_client()
    futures = {}
...
def main(args=None):
    ...
    if not no_github_fetch:
        gx = close_labels(args.dry_run, gx)
        gx = update_graph_pr_status(args.dry_run, gx)
