...
def _process_section(name, attrs, lines):
    ...
    is_boost_in_build = any(x.startswith("boost-cpp") for x in build_req)
    is_boost_in_host = any(x.startswith("boost-cpp") for x in host_req)
    is_boost_in_run = any(x.startswith("boost-cpp") for x in run_req)
